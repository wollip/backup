...
public class Application extends JFrame {
    ...
    private void doStuff() {
        ...
        for (Cube cube : cubes) {
            ...
            for (int i=0; i<8; i++) {
                ...
                points[i] = new Point((int) (v.get(0) + CANVAS_WIDTH/2), (int) (-v.get(1) + CANVAS_HEIGHT/2));
            }
    ...
    public static Matrix getRotationTransformMatrix(Vector normal) {

        //Vector t = angle.multiply(-1);
        
        double angleX, angleY, angleZ;
        
        angleX = Math.atan2(normal.get(2), normal.get(1));
        angleX = (angleX + 360)%360;
        angleX = Math.toRadians(angleX);
        Matrix cameraTransformSuppX = new Matrix(new double[][] {
        	{1,0,0},
        ...
        /*
        Matrix cameraTransformSuppZ = new Matrix(new double[][] {
        ...
        */
        ...
        angleY = Math.atan2(normal.get(2), normal.get(0));
        angleY = (angleY+360)%360;
        angleY = Math.toRadians(angleY);
        Matrix cameraTransformSuppY = new Matrix(new double[][] {
                {Math.cos(angleY), 0, -Math.sin(angleY)},
                {0, 1, 0},
                {Math.sin(angleY), 0, Math.cos(angleY)}
        });

        return cameraTransformSuppX.multiply(cameraTransformSuppY);
    }
